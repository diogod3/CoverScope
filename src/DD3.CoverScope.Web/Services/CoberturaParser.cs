...
public sealed class CoberturaParser
{
    public CoverageReport Parse(string reportPath, string? sourceRoot = null)
    {
        ...
        using var stream = File.OpenRead(absoluteReportPath);
        ...
        var timestamp = ParseTimestamp(root.Attribute("timestamp")?.Value)
            ?? new DateTimeOffset(File.GetLastWriteTimeUtc(absoluteReportPath));
    ...
    public IReadOnlyList<SourceLine> ReadSource(FileCoverage file)
    {
        if (file.ResolvedPath is null || !File.Exists(file.ResolvedPath)) return [];
        var coverage = file.Lines.ToDictionary(x => x.Number);
        return File.ReadLines(file.ResolvedPath)
    ...
    private static ParsedClass ParseClass(XElement element, IReadOnlyList<string> roots, string reportDirectory)
    {
    ...
    private static string? ResolveSourcePath(string path, IReadOnlyList<string> roots, string reportDirectory)
    {
    ...
    private sealed record ParsedClass(string Package, ClassCoverage Class);
}
