...
public partial class CoverageImportService
{
    ...
    private async ValueTask<T> TryCatch<T>(Func<ValueTask<T>> operation)
    {
        ...
        { throw new CoverageOperationDependencyValidationException(nameof(CoverageImportService), exception.InnerException ?? exception); }
        catch (CoverageOperationException exception)
        { throw new CoverageOperationDependencyException(nameof(CoverageImportService), exception.InnerException ?? exception); }
        ...
        { throw new CoverageOperationValidationException(nameof(CoverageImportService), exception); }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException
            or System.ComponentModel.Win32Exception or System.Security.SecurityException)
        { throw new CoverageOperationDependencyException(nameof(CoverageImportService), exception); }
        catch (Exception exception)
        { throw new CoverageOperationServiceException(nameof(CoverageImportService), exception); }
    }
}
