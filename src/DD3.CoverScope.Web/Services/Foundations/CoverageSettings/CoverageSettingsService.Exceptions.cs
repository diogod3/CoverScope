...
public partial class CoverageSettingsService
{
    ...
    private async ValueTask<T> TryCatch<T>(Func<ValueTask<T>> operation)
    {
        ...
        { throw new CoverageOperationDependencyValidationException(nameof(CoverageSettingsService), exception.InnerException ?? exception); }
        catch (CoverageOperationException exception)
        { throw new CoverageOperationDependencyException(nameof(CoverageSettingsService), exception.InnerException ?? exception); }
        ...
        { throw new CoverageOperationValidationException(nameof(CoverageSettingsService), exception); }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException
            or System.ComponentModel.Win32Exception or System.Security.SecurityException)
        { throw new CoverageOperationDependencyException(nameof(CoverageSettingsService), exception); }
        catch (Exception exception)
        { throw new CoverageOperationServiceException(nameof(CoverageSettingsService), exception); }
    }
}
