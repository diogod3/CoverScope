...
public partial class TestRunSummaryService
{
    ...
    private async ValueTask<T> TryCatch<T>(Func<ValueTask<T>> operation)
    {
        ...
        { throw new CoverageOperationDependencyValidationException(nameof(TestRunSummaryService), exception.InnerException ?? exception); }
        catch (CoverageOperationException exception)
        { throw new CoverageOperationDependencyException(nameof(TestRunSummaryService), exception.InnerException ?? exception); }
        ...
        { throw new CoverageOperationValidationException(nameof(TestRunSummaryService), exception); }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException
            or System.ComponentModel.Win32Exception or System.Security.SecurityException)
        { throw new CoverageOperationDependencyException(nameof(TestRunSummaryService), exception); }
        catch (Exception exception)
        { throw new CoverageOperationServiceException(nameof(TestRunSummaryService), exception); }
    }
}
