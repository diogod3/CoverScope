...
public partial class TestRunSummaryService : ITestRunSummaryService
{
    public TestRunSummary Parse(IEnumerable<string> reportPaths) => Trace(() => TryCatch(() =>
    {
        ValidateParse(reportPaths);
        return ValueTask.FromResult(ParseCore(reportPaths));
    })).GetAwaiter().GetResult();

    private readonly IFileSystemBroker fileSystemBroker;
    private readonly IDiagnosticsBroker diagnosticsBroker;

    public TestRunSummaryService(IFileSystemBroker fileSystemBroker, IDiagnosticsBroker diagnosticsBroker)
    {
        this.fileSystemBroker = fileSystemBroker;
        this.diagnosticsBroker = diagnosticsBroker;
    }

    private TestRunSummary ParseCore(IEnumerable<string> reportPaths)
    {
        var passed = 0;
        var failed = 0;
        var skipped = 0;
        ...
        {
            var document = XDocument.Parse(fileSystemBroker.ReadAllText(reportPath), LoadOptions.None);
    ...
    private record TestDefinition(string Id, string ClassName, string MethodName, string CodeBase);
}
