...
public partial class CoverageWorkspaceViewService
{
    ...
    private async ValueTask<T> TryCatch<T>(Func<ValueTask<T>> operation)
    {
        ...
        { throw new CoverageOperationDependencyValidationException(nameof(CoverageWorkspaceViewService), exception.InnerException ?? exception); }
        catch (CoverageOperationException exception)
        { throw new CoverageOperationDependencyException(nameof(CoverageWorkspaceViewService), exception.InnerException ?? exception); }
        ...
        { throw new CoverageOperationValidationException(nameof(CoverageWorkspaceViewService), exception); }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException
            or System.ComponentModel.Win32Exception or System.Security.SecurityException)
        { throw new CoverageOperationDependencyException(nameof(CoverageWorkspaceViewService), exception); }
        catch (Exception exception)
        { throw new CoverageOperationServiceException(nameof(CoverageWorkspaceViewService), exception); }
    }
}
