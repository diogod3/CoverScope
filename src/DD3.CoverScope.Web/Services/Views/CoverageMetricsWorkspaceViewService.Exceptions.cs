...
public partial class CoverageMetricsWorkspaceViewService
{
    ...
    private async ValueTask<T> TryCatch<T>(Func<ValueTask<T>> operation)
    {
        ...
        { throw new CoverageOperationDependencyValidationException(nameof(CoverageMetricsWorkspaceViewService), exception.InnerException ?? exception); }
        catch (CoverageOperationException exception)
        { throw new CoverageOperationDependencyException(nameof(CoverageMetricsWorkspaceViewService), exception.InnerException ?? exception); }
        ...
        { throw new CoverageOperationValidationException(nameof(CoverageMetricsWorkspaceViewService), exception); }
        catch (Exception exception) when (exception is IOException or UnauthorizedAccessException
            or System.ComponentModel.Win32Exception or System.Security.SecurityException)
        { throw new CoverageOperationDependencyException(nameof(CoverageMetricsWorkspaceViewService), exception); }
        catch (Exception exception)
        { throw new CoverageOperationServiceException(nameof(CoverageMetricsWorkspaceViewService), exception); }
    }
}
