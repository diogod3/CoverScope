...
public sealed class CoverageRunner
{
        ...
        IProgress<CoverageCollectionPhase>? progress = null)
    {
        ...
        var resultsDirectory = Path.Combine(workingDirectory, "coverage-output", DateTime.UtcNow.ToString("yyyyMMdd-HHmmss-fff"));
        ...
        try
        {
            ...
            return new(CoverageRunOutcome.Succeeded, $"Coverage collected successfully from {scope}.", output, report, tests);
        }
        catch (System.ComponentModel.Win32Exception ex)
        {
            return new(CoverageRunOutcome.ExecutionFailed, ".NET 10 SDK was not found. Install .NET 10 and ensure dotnet is on PATH.", ex.Message);
        }
        ...
        {
            return new(CoverageRunOutcome.ExecutionFailed, "Coverage collection failed.", ex.Message);
        }
    }
}
